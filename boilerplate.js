const BOILERPLATE = {
    ...
    localisation: (address, lat, lon) => {
        ...
        if (lat && lon) {
            ...
            // Image statique via staticmap.openstreetmap.de — fonctionne en preview inline et à l'impression
            const staticMapUrl = `https://staticmap.openstreetmap.de/staticmap.php?center=${latF},${lonF}&zoom=16&size=600x300&markers=${latF},${lonF},ol-marker`;
            mapBlock = `
            <div style="border:2px solid #3b82f6;border-radius:12px;overflow:hidden;box-shadow:0 4px 12px rgba(59,130,246,0.12);margin-bottom:10px;">
                ...
                <div style="background:linear-gradient(135deg,#eff6ff 0%,#dbeafe 50%,#bfdbfe 100%);padding:18px 20px;display:flex;align-items:center;gap:14px;">
                    ...
                    <div style="margin-left:auto;font-size:0.65rem;color:#93c5fd;font-weight:700;letter-spacing:0.5px;white-space:nowrap;">LOCALISATION GPS</div>
                </div>
                <!-- Carte statique OSM — image, pas iframe, compatible print et preview -->
                <a href="${mapsUrlCoords}" target="_blank" rel="noopener noreferrer" style="display:block;line-height:0;">
                    <img src="${staticMapUrl}" width="100%" alt="Carte — ${address}"
                         style="display:block;width:100%;height:auto;min-height:200px;background:linear-gradient(135deg,#eff6ff,#dbeafe);"
                         onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
                    <div style="display:none;height:200px;align-items:center;justify-content:center;background:linear-gradient(135deg,#eff6ff,#dbeafe);flex-direction:column;gap:12px;">
                        <div style="font-size:2.5rem;">🗺️</div>
                        <div style="font-size:0.9rem;color:#1e40af;font-weight:600;">Ouvrir dans Google Maps</div>
                    </div>
                </a>
                <!-- Lien Google Maps -->
        ...
        <div class="page-break" style="padding-top: 50px;">
            ...
            ${mapBlock}
            <p style="font-style:italic;color:#64748b;font-size:0.85rem;">* Carte générée via OpenStreetMap. Cliquer pour ouvrir dans Google Maps.</p>
        </div>
        `;
    },
